...
public class LocalGroupShortName extends ChatComponent implements HoverComponent {
	KaranteeniPerms perms;
	public LocalGroupShortName(KaranteeniPerms plugin, String key) {
		super(key);
		perms = plugin;
	}
	...
	public HashMap<Player, BaseComponent> getHover(Player sender, Set<Player> rec) {
		...
		for(Player player : rec)
			comps.put(player, new TextComponent(TextComponent.fromLegacyText(group.getShortName(player))));
