...
public class OnLightning implements Listener{
	...
	public OnLightning()
	{
		plugin = Katura.getPlugin(Katura.class);
		//Save a config option for the lightning effects
		if(!plugin.getConfig().isSet(configKey)) {
			plugin.getConfig().set(configKey, true);
			plugin.saveConfig();
		}
		
		if(!plugin.getConfig().isSet(configKey2)) {
			plugin.getConfig().set(configKey2, 4);
			plugin.saveConfig();
		}
	}
	...
	private void onLightning(LightningStrikeEvent event) {
		...
		// This works only with weather or lightning summon
	...
	private void switchType(Block block, Material material) {
		// check if the lightning is allowed to transform blocks
		if(plugin.getWorldGuardManager() != null && 
				!plugin.getWorldGuardManager().canLightningTransform(block.getLocation()))
			return;
