...
public class TpAskHere extends AbstractCommand implements TranslationContainer {
	public TpAskHere(Karanteenials plugin) {
		super(plugin, "tpahere", "/tpahere <player>", "Asks for permission to teleport player to you", 
	...
	public boolean onCommand(CommandSender sender, Command arg1, String arg2, String[] args) {
		...
		Karanteenials.getMessager().sendMessage(player, Sounds.NONE.get(),
				Prefix.NEUTRAL+
				Karanteenials.getTranslator().getTranslation(plugin, player, "teleport-ask.asker.here.sent")
				.replace("%player%", destination.getName()));
		...
		Karanteenials.getMessager().sendMessage(destination, Sounds.NONE.get(),
				Prefix.NEUTRAL+
				Karanteenials.getTranslator().getTranslation(plugin, destination, "teleport-ask.receiver.here.received")
				.replace("%player%", player.getName()));
		
		TeleportRequest req = new TeleportRequest(KPlayer.getKPlayer(player), KPlayer.getKPlayer(destination), TeleportType.HERE);
	...
	public List<String> onTabComplete(CommandSender sender, Command arg1, String arg2, String[] args) {
		if(sender.hasPermission("karanteenials.teleport.askhere") && args.length == 1)
	...
	public void registerTranslations() {
		Karanteenials.getTranslator().registerTranslation(plugin, "teleport-ask.asker.here.sent", 
				"You sent a teleport request to %player% to teleport to you");
		Karanteenials.getTranslator().registerTranslation(plugin, "teleport-ask.receiver.here.received", 
				"%player% wants you to teleport to them, /tpyes to accept and /tpno to deny. You have 60 seconds to reply");
