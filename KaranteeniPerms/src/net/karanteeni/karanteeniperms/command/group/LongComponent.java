...
public class LongComponent extends CommandComponent implements TranslationContainer {
	...
	protected CommandResult runComponent(CommandSender sender, Command cmd, String label, String[] args) {
		...
		// set group name
		group.setName(locale, name, true);
		
		// message about change
		KaranteeniPerms.getMessager().sendMessage(sender, Sounds.SETTINGS.get(), Prefix.POSITIVE +
				KaranteeniPerms.getTranslator().getTranslation(this.chainer.getPlugin(), sender, "permissions.group.set.rankname-long")
	...
	public void registerTranslations() {
		KaranteeniPlugin.getTranslator().registerTranslation(
				this.chainer.getPlugin(), "permissions.group.set.rankname-long", 
				"Set the long name of group %group% in language %language% to §r%name%");
	}
}
