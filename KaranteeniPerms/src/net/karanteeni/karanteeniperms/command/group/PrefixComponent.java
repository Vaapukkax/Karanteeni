...
public class PrefixComponent extends CommandComponent implements TranslationContainer {
	...
	protected CommandResult runComponent(CommandSender sender, Command cmd, String label, String[] args) {
		...
		// message player about the new prefix
		KaranteeniPerms.getMessager().sendMessage(sender, Sounds.SETTINGS.get(), Prefix.POSITIVE +
				KaranteeniPerms.getTranslator().getTranslation(this.chainer.getPlugin(), sender, "permissions.group.set.prefix")
				.replace("%group%", group.getID())
				.replace("%prefix%", groupName));
		
		// message if the prefix is malformed
		if(!groupName.contains("%group%"))
		KaranteeniPerms.getMessager().sendMessage(sender, Sounds.NONE.get(), Prefix.NEGATIVE +
				KaranteeniPerms.getTranslator().getTranslation(this.chainer.getPlugin(), sender, "permissions.group.tag-recommended"));
	...
	public void registerTranslations() {
		KaranteeniPlugin.getTranslator().registerTranslation(
				this.chainer.getPlugin(), "permissions.group.tag-recommended",
				"It is recommended to use tag %group% in prefix to allow "
						+ "translation engine input the translated group name into prefix!");
		KaranteeniPlugin.getTranslator().registerTranslation(
				this.chainer.getPlugin(), "permissions.group.set.prefix", 
				"Prefix of group %group% was set to §r%prefix%");
	}
}