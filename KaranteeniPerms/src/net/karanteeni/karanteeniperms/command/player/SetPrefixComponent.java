...
public class SetPrefixComponent extends CommandComponent implements TranslationContainer {
	...
	protected CommandResult runComponent(CommandSender sender, Command cmd, String label, String[] args) {
		...
		KaranteeniPerms.getMessager().sendMessage(sender, Sounds.SETTINGS.get(), 
				Prefix.POSITIVE + KaranteeniPerms.getTranslator().getTranslation(this.chainer.getPlugin(), sender, 
						"permissions.player.set.prefix").replace("%player%", args[0]).replace("%prefix%", name));
		
		// if the prefix does not contain group tag recommend its usage
		if(!name.contains("%group%")) {
			KaranteeniPerms.getMessager().sendMessage(sender, Sounds.NONE.get(), 
					Prefix.NEUTRAL + KaranteeniPerms.getTranslator().getTranslation(this.chainer.getPlugin(), sender, 
							"permissions.group.tag-recommended"));
		}
	...
	public void registerTranslations() {
		KaranteeniPerms.getTranslator().registerTranslation(this.chainer.getPlugin(), 
				"permissions.player.set.prefix", 
				"Set the prefix of %player% to %prefix%");
	}
}
