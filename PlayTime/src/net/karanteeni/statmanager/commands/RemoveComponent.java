...
public class RemoveComponent extends CommandComponent implements TranslationContainer {
	...
	protected CommandResult runComponent(CommandSender sender, Command arg1, String arg2, String[] args) {
		...
		playerTime.subtract(timeToSet.getMilliseconds());
		if(!playerTime.save()) return CommandResult.ERROR;
		
		StatManager.getMessager().sendMessage(sender, Sounds.SETTINGS.get(), 
				Prefix.NEUTRAL +
				StatManager.getTranslator().getTranslation(this.chainer.getPlugin(), sender, "subtract-time")
	...
	public void registerTranslations() {
		StatManager.getTranslator().registerTranslation(this.chainer.getPlugin(), 
				"subtract-time", "Removed %time% form %player%s play time");
	}
}
